...
namespace JxlSharp
{
    ...
    internal unsafe partial class UnsafeNativeJxl
    {
        ...
        static IntPtr TryLoadLibrary(string arch, string inputDllName)
        {
            ...
            string libPath = Path.Combine(assemblyPath, "dependencies/jxl");
        ...
        static UnsafeNativeJxl()
        {
            ...
            brotliCommonModule = TryLoadLibrary(brotliCommonDllName);
            brotliDecModule = TryLoadLibrary(brotliDecDllName);
            brotliEncModule = TryLoadLibrary(brotliEncDllName);
            //load jxl, and jxl_threads
            libJxlModule = TryLoadLibrary(jxlDllName);
            libJxlThreadModule = TryLoadLibrary(jxlThreadsDllName);
        ...
        public static bool AreLibrariesLoaded()
        {
            return libJxlModule != IntPtr.Zero && libJxlThreadModule != IntPtr.Zero && brotliCommonModule != IntPtr.Zero && brotliDecModule != IntPtr.Zero && brotliEncModule != IntPtr.Zero;
        }
    }
}